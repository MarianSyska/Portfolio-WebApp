...
class TestBasePageClean:
    ...
    def test_clean_raises_when_in_menu_no_icon(self, db, admin_user, wagtail_image):
        ...
        page.menu_icon = ""
        with pytest.raises(ValidationError, match="Menu Icon must be set"):
            page.clean()
...
class TestBasePageGetContext:
    ...
    def test_context_has_menu_items(self, rf, db, admin_user, root_page, wagtail_image):
        """Check that the menu items are correctly set based on the show_in_menus flag."""
        ...
        show_in_menu_series = [True, False, False, True, True]
        dummy_pages_pks = []
        for i, show_in_menu in enumerate(show_in_menu_series):
            dummy_page = BasePage(title=f"Test{i}",
                                  user=admin_user,
                                  show_in_menus=show_in_menu,
                                  menu_icon="icon" if show_in_menu else "",
                                  avatar=wagtail_image)
            root_page.add_child(instance=dummy_page)
            dummy_pages_pks.append(dummy_page.pk)

        request = HttpRequest()
        request.GET = {}
        context = page.get_context(request)

        actual_menu_item_pks = set(item.pk for item in context["menu_items"])
        expected_menu_item_pks = set(itertools.compress(dummy_pages_pks,
                                                        show_in_menu_series))
        expected_menu_item_pks.add(3)

        assert "menu_items" in context
        assert (actual_menu_item_pks == expected_menu_item_pks)
